'''
功能：
    1.监控目标文件夹，若该文件夹内有.bz2文件新增，则开始运行转换程序
    2.每隔一段时间运行历史数据删除程序(删除转化得到的.ar2v数据)
使用方法：
    1.修改io.ini文件里对应的路径。程序会仅把该路径下的子一级文件夹内新增的.bz文件,
    转换成.ar2v并保存至output路径文件夹内，并按雷达站名区分
    2.修改io.ini文件删除任务参数。其中output路径文件夹内在当前时间 DELTA_TIME 天前的资料将被删除，
    以及删除任务将每 SLEEP_TIME 秒运行一次
注意：
    1.程序只能转换原始数据为标准格式的雷达资料
    2.当前程序可以转换的雷达站分别有：南昌,九江,上饶,福州,宜春,吉安,赣州,景德镇,
    武汉,岳阳,长沙,衡阳,郴州,韶关,梅州,汕头,安庆,黄山,铜陵,三明,衢州
作者: 江西省气象台卢文旭
'''
import os
import read_bin
